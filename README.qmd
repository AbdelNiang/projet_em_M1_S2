...
```
L'opérateur de projection fait perdre de l'information → problème inverse mal posé

Génération d'un signal de test :
On condidére un signal 2D gaussien :
```python
p = 40
...
plt.show()
```
On observe des signaux 1D projetés($Y_i = A_{z_i} \theta$) : prjection d'une gaussienne 2D → gaussienne 1D :
